...
class Permohonan_security extends CI_Controller
{
    ...
    public function json()
    {
        ...
        echo $this->Permohonan_security_model->json();
    }
    ...
    public function update($id)
    {
        ...
        if ($row) {
            $data = array(
                ...
                'id_mobil' => set_value('jenis', $row->nopol),
                'kupon_bbm' => set_value('jenis', $row->kupon_bbm),
                'id_driver' => set_value('jenis', $row->driver),
                'bbm' => set_value('bbm', $row->bbm),
                'is_driver' => set_value('is_driver', $row->is_driver),
                'status' => set_value('status', $row->status),
                'datetime' => set_value('datetime', $row->datetime),
            );
            $this->template->load('template', 'permohonan_security/permohonan_security_form', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('permohonan_security'));
        }
    }
