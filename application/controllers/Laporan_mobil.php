...
class Laporan_mobil extends CI_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('Laporan_mobil_model');
        $this->load->library('form_validation');
        $this->session->set_flashdata('title', 'Laporan Mobil | PT PLN');
    ...
    public function index()
    {
        ...
        $this->template->load('template', 'laporan_mobil/laporan');
    }

    public function ajax_list()
    {
        $list = $this->Laporan_mobil_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $sum = 0;
        foreach ($list as $customers) {
            ...
            $row[] = $customers->mobil;
            $row[] = $customers->nopol;
            $row[] = $customers->keluar_jam;
            $row[] = $customers->masuk_jam;
            $row[] = rename_string_status_mobil($customers->status);
            // $row[] = '<a class="btn btn-sm btn-primary" href="' . base_url('laporan_mobil/print/' . $customers->notrans) . '" title="Edit" target="_blank">Print</a>';
            $data[] = $row;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Laporan_mobil_model->count_all(),
            "recordsFiltered" => $this->Laporan_mobil_model->count_filtered(),
            "data" => $data,
            // "sum"             => $sum,
        );
    ...
    public function read($no_po)
    {
        ...
        $this->template->load('template', 'laporan_mobil/laporan_read', $data);
    ...
    public function pdf()
    {
        ...
        $nourut = 1;
        foreach ($this->Laporan_mobil_model->excel($tanggal_a, $tanggal_b) as $data) {
    ...
    public function report()
    {
        ...
        $this->pdfgenerator->load_view('laporan_mobil/laporan_pdf', $data);
    }
    ...
    function print($no_po) {
        ...
        $this->pdfgenerator->setPaper('A4', 'landscape');
        $this->pdfgenerator->filename = "laporan.pdf";
        $this->pdfgenerator->load_view('laporan_mobil/laporan_print', $data);
    }
